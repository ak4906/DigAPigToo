    ...
    private func createStructures() -> [AnatomyStructure] {
        ...
        if let externalCat = categories.first(where: { $0.name == "External" }) {
            structures.append(contentsOf: [
                AnatomyStructure(
                    ...
                    function: "The flattened, disc-shaped snout at the very rostral end of the pig; used for rooting and searching through soil for food; highly touch-sensitive. The rostral plate is the entire snout disk — it CONTAINS the external nostrils (the two openings/holes within it) but is not the same as the nostrils themselves.",
                    commonConfusions: ["Rostral plate vs external nostrils: the rostral plate is the whole snout disk; the external nostrils are just the two holes/openings within that disk — they are part of the rostral plate, not the same thing"],
                    examTips: ["Snout disk at the very rostral end of pig — the whole flat disc", "The external nostrils are the two openings inside it, not the plate itself"],
                    images: [ImageCDN.image("rostral-plate_gross_1.jpeg", caption: "Rostral Plate")],
                ...
                AnatomyStructure(
                    ...
                    function: "The two openings/holes within the rostral plate through which air enters the nasal cavity; allow airflow for respiration and olfaction; air is warmed, humidified, and filtered as it passes through.",
                    commonConfusions: ["External nostrils vs rostral plate: the nostrils are the two holes; the rostral plate is the entire snout disk that surrounds and contains them — students sometimes label the whole snout as 'nostril' or vice versa"],
                    examTips: ["Two openings at tip of snout within the rostral plate — the holes, not the disk"],
        ...
        if let circulatoryCat = categories.first(where: { $0.name == "Circulatory System" }) {
            structures.append(contentsOf: [
                ...
                AnatomyStructure(
                    categoryId: circulatoryCat.id,
                    name: "Internal Thoracic Arteries",
                ...
                AnatomyStructure(
                    categoryId: circulatoryCat.id,
                    name: "Jejunal Arteries",
                    aliases: ["Jejunal arterial branches"],
                ...
                AnatomyStructure(
                    ...
                    aliases: ["Gastroepiploic arterial supply"],
                ...
                AnatomyStructure(
                    categoryId: circulatoryCat.id,
                    name: "Splenic Artery",
                    aliases: ["Lienic artery", "Splenic/lienic artery"],
                ...
                AnatomyStructure(
                    categoryId: circulatoryCat.id,
                    name: "Deep Circumflex Iliac Artery",
                ...
                AnatomyStructure(
                    categoryId: circulatoryCat.id,
                    name: "Ovarian Artery",
                    aliases: ["Gonadal artery (female)", "Ovarian arterial supply"],
                ...
                AnatomyStructure(
                    categoryId: circulatoryCat.id,
                    name: "Ovarian Vein",
                    aliases: ["Gonadal vein (female)", "Ovarian venous drainage"],
        ...
        if let maleRepCat = categories.first(where: { $0.name == "Male Reproductive" }) {
            structures.append(contentsOf: [
                ...
                AnatomyStructure(
                    ...
                    function: "Produce ALKALINE secretions that lubricate during copulation and help neutralize the acidity of the vagina, improving sperm survival. Paired glands lying on the DORSAL side of the urethra.",
                    commonConfusions: ["Bulbourethral vs preputial glands: both give alkaline lubricating secretions, but bulbourethral glands sit on the dorsal urethra, while the preputial gland is distal along the penis (often lost in dissection)", "One of the >60%-of-seminal-fluid contributors"],
                    examTips: ["Practical ID: paired glands on the DORSAL side of the urethra (deep in the pelvic cavity, near the rectum)", "Alkaline secretion → lubrication + neutralizes vaginal acidity", "Exocrine glands — secretions travel through ducts into urethra"],
                    images: [
                ...
                AnatomyStructure(
                    ...
                    function: "Secrete a viscous fluid rich in FRUCTOSE (energy for sperm motility/viability), mucus (keeps sperm from drying out), and hormones that stimulate uterine contractions to move sperm along the female tract. The secretion is highly ALKALINE, neutralizing the acidic vagina to improve sperm survival.",
                    commonConfusions: ["Fructose is the key seminal-vesicle product — it fuels sperm motility", "Together the seminal vesicles, prostate, bulbourethral glands, and preputial glands contribute >60% of total seminal fluid volume"],
                    examTips: ["Practical ID: located at the dorsal juncture of the ductus deferens and urethra (may be hard to find on your specimen)", "Alkaline + fructose-rich + mucus = the seminal vesicle secretion", "Exocrine glands — folded glandular mucosa increases secretory surface area"],
                    images: [
                        ImageCDN.image("seminal-vesicles_gross_1.png", caption: "Seminal Vesicles"),
                    ],
                    ...
                    connections: "Located at the dorsal junction of the ductus deferens and urethra; secretions enter the reproductive tract there",
                    highYield: true
                ),
                AnatomyStructure(
                    categoryId: maleRepCat.id,
                ...
                AnatomyStructure(
                    ...
                    function: "Delivers semen during ejaculation and passes urine — both travel through the URETHRA, which runs lengthwise THROUGH the penis. The urethra is the tube; the penis is the erectile organ carrying it to the exterior.",
                    commonConfusions: ["Penis vs urethra: the urethra is a single tube running the whole length INSIDE the penis; during ejaculation, seminal fluid + sperm mix and travel through this urethra along the penis to exit at the tip (urogenital orifice)", "In the FETAL pig the penis does NOT protrude — it is enclosed in an epithelial sheath and lies along the ventral abdominal wall (find it by tracing back from the preputial orifice)", "The base of the penis is enlarged by the bulbocavernous muscle, which helps expel urine and seminal fluid"],
                    examTips: ["Practical ID in fetal pig: not external/protruding — a firm cord under the skin running back from the preputial orifice along the ventral midline", "The urethra runs through the penis carrying BOTH urine and semen", "Follow: urethra (inside penis) → tip of penis → preputial orifice → exterior"],
                    images: [
                    ...
                    connections: "Urethra (runs through penis) → penis tip → preputial orifice → exterior",
                    highYield: true
                ),
                AnatomyStructure(
                    categoryId: maleRepCat.id,
        ...
        if let giHistologyCat = categories.first(where: { $0.name == "Gastrointestinal Histology" }) {
            structures.append(contentsOf: [
                AnatomyStructure(categoryId: giHistologyCat.id, name: "Mucosa (Esophagus)", aliases: ["Esophageal mucosa", "Stratified squamous epithelium (esophagus)", "Mucous membrane (esophagus)"],
                    function: "Innermost wall layer (a 'mucous membrane'). IMPORTANT: the mucosa is not just the epithelium — it is THREE sublayers together: (1) epithelium — a thick stratified squamous epithelium of 40–50 cell layers for protection during swallowing; (2) lamina propria — loose connective tissue that supports and contains blood vessels, nerves, and immune cells; (3) muscularis mucosae — a thin smooth-muscle layer that moves the mucosa to aid digestion/absorption.",
                    commonConfusions: ["Mucosa = epithelium + lamina propria + muscularis mucosae (all three) — not just the epithelium", "The epithelium is separated from the lamina propria by a thin BASEMENT MEMBRANE, visible as a dark band beneath the epithelium", "It's called a 'mucous membrane' because gut epithelium characteristically produces mucus"],
                    examTips: ["KEY esophagus feature: thick stratified squamous epithelium (40–50 cell layers)", "Find the three mucosal sublayers: epithelium → lamina propria → muscularis mucosae", "Look for the basement membrane as a dark band just under the squamous epithelium", "Thick stratified squamous = enhanced protection from abrasion during swallowing"],
                    images: [ImageCDN.slide("mucosa-esophagus_histo_1.jpeg", magnification: 4, caption: "Mucosa (Esophagus) — 4×")],
                    histology: "Three sublayers: (1) stratified squamous epithelium (40–50 cell layers) sitting on a thin basement membrane; (2) lamina propria (loose connective tissue with vessels, nerves, immune cells); (3) muscularis mucosae (thin smooth muscle)"),
                AnatomyStructure(categoryId: giHistologyCat.id, name: "Muscularis Mucosae (Esophagus)", aliases: ["Muscularis mucosa (esophagus)"],
                    function: "The third of the three mucosal sublayers — a thin layer of smooth muscle that helps with the local movement of the mucosa to facilitate digestion and absorption. It forms the base of the mucosa, separating it from the submucosa.",
                    commonConfusions: ["Muscularis MUCOSAE (thin, part of the mucosa) vs the much thicker MUSCULARIS — also called the muscularis externa or muscularis propria — which is the inner circular + outer longitudinal coat that drives peristalsis, deeper in the wall. Similar names, completely different layers.", "It is the deepest of the three mucosal sublayers and the boundary between mucosa and submucosa"],
                    examTips: ["Thin smooth-muscle line at the base of the mucosa", "Don't confuse with the thick MUSCULARIS (= muscularis externa = muscularis propria) deeper in the wall", "Everything luminal to it = mucosa; everything deep to it = submucosa and beyond"],
                    images: [ImageCDN.slide("muscularis-mucosae-esophagus_histo_1.jpeg", magnification: 4, caption: "Muscularis Mucosae (Esophagus) — 4×")],
                    histology: "Thin layer of smooth muscle at the base of the mucosa"),
                AnatomyStructure(categoryId: giHistologyCat.id, name: "Submucosa (Esophagus)", aliases: ["Esophageal submucosa"],
                    function: "A layer of irregular connective tissue directly below the mucosa that links the mucosa to the underlying muscularis. It contains blood and lymphatic vessels, a branching network of nerves, and submucosal mucus glands with their ducts.",
                    commonConfusions: ["KEY FACT: within the entire digestive tract, ONLY the esophagus and the duodenum have glands in the submucosa — this is a major identifying feature", "Submucosa (connective tissue, deep to the mucosa) vs muscularis mucosae (thin muscle inside the mucosa) — different layers"],
                    examTips: ["Look deep to the muscularis mucosae for mucus glands + ducts = submucosal glands (esophagus/duodenum clue)", "Submucosa also carries the larger blood/lymphatic vessels and a branching nerve network"],
                    images: [ImageCDN.slide("submucosa-esophagus_histo_1.jpeg", magnification: 4, caption: "Submucosa (Esophagus) — 4×")],
                    histology: "Irregular connective tissue containing submucosal mucus glands and ducts, blood and lymphatic vessels, and a branching nerve network"),
                AnatomyStructure(categoryId: giHistologyCat.id, name: "Inner Muscle Layer (Esophagus)", aliases: ["Inner circular layer (esophagus)"],
                    function: "Inner CIRCULAR layer of the muscularis — the thick muscle coat (also called the muscularis externa or muscularis propria) whose two layers, inner circular + outer longitudinal, contract to drive peristalsis, moving the food bolus along the tract. The upper third of the esophagus contains skeletal muscle, allowing voluntary control over the initiation of swallowing.",
                    commonConfusions: ["This is the MUSCULARIS — also known as the muscularis externa or muscularis propria (same layer, three names): thick, deep in the wall, drives peristalsis. Do NOT confuse it with the thin muscularis MUCOSAE up inside the mucosa.", "Upper third of the esophagus = skeletal muscle (voluntary initiation of swallowing)", "Muscle-layer ORIENTATION can appear to shift around the circumference on a slide — this just reflects the position of the tissue when it was cut, not a real change in the layer"],
                    examTips: ["Inner circular = fibers cut so they encircle the lumen (ring-like)", "Muscularis (= muscularis externa = muscularis propria) = inner circular + outer longitudinal; their contraction = peristalsis"],
                    images: [ImageCDN.slide("inner-muscle-layer-esophagus_histo_1.jpeg", magnification: 4, caption: "Inner Muscle Layer (Esophagus) — 4×")],
                    histology: "Inner circular layer of the muscularis (smooth muscle, with skeletal muscle in the upper third of the esophagus)"),
                AnatomyStructure(categoryId: giHistologyCat.id, name: "Outer Muscle Layer (Esophagus)", aliases: ["Outer longitudinal layer (esophagus)"],
                    function: "Outer LONGITUDINAL layer of the muscularis (also called the muscularis externa or muscularis propria). Together with the inner circular layer, its contractions drive peristalsis (and constriction), moving the food bolus down the tract. The upper third of the esophagus contains skeletal muscle for voluntary control of swallowing.",
                    commonConfusions: ["Outer longitudinal vs inner circular — in cross-section, longitudinal fibers are cut lengthwise (elongated profiles) while circular fibers appear as rings", "This is part of the MUSCULARIS — also known as the muscularis externa or muscularis propria (same layer, three names) — NOT the thin muscularis mucosae up in the mucosa", "Apparent shifts in muscle orientation around the circumference just reflect how the tissue was cut"],
                    examTips: ["Outer longitudinal + inner circular together = the two-layer muscularis (= muscularis externa = muscularis propria) that produces peristalsis", "This two-layer pattern is clearest in the small intestine; the stomach adds a third (oblique) layer"],
                    images: [ImageCDN.slide("outer-muscle-layer-esophagus_histo_1.jpeg", magnification: 4, caption: "Outer Muscle Layer (Esophagus) — 4×")],
                    histology: "Outer longitudinal layer of the muscularis (smooth muscle, with skeletal muscle in the upper third of the esophagus)"),
                AnatomyStructure(categoryId: giHistologyCat.id, name: "Adventitia (Esophagus)", aliases: ["Esophageal adventitia"],
                    function: "Outermost layer of the esophagus — a dense sheath of collagen fibers that anchors the esophagus near the ventral surface of the vertebral column. In the thorax (respiratory system and esophagus), the outermost layer is adventitia rather than serosa; the mouth and pharynx also have adventitia.",
                    commonConfusions: ["Adventitia (dense collagen sheath, anchors thoracic organs) vs serosa (visceral peritoneum, found on abdominal digestive organs) — the esophagus has adventitia", "In the digestive tract, serosa appears on the abdominal organs; the thoracic esophagus keeps adventitia as its outer layer"],
                    examTips: ["Outermost dense collagenous layer with no mesothelial covering = adventitia", "Mouth, pharynx, and esophagus all use adventitia (not serosa) as the outer layer"],
                    images: [ImageCDN.slide("adventitia-esophagus_histo_1.jpeg", magnification: 4, caption: "Adventitia (Esophagus) — 4×")],
                    histology: "Dense sheath of collagen fibers (with vessels and nerves) that anchors the esophagus toward the ventral vertebral column; no mesothelial lining"),
            ])
        }
